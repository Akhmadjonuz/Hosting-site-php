...
echo '<div class="title">VDS | Перезагрузка VDS</div>';
if ($parse_xmla->osinstalled == 'on'){
if (empty($_POST['submit'])) {
    echo '<div class="menu">';
        echo '<form action="" method="post">' . "\n";
        echo '<b>Перезагрузка VDS ?</b>';
		...
		$qas=array("ssl"=>array("verify_peer"=>false,"verify_peer_name"=>false,),);
        $content = file_get_contents("https://".$server."/vmmgr?authinfo=".$vlogin.":".$vpass."&out=xml&func=vm.restart&elid=".$user['vds_id']."&sok=ok", false, stream_context_create($qas));
